...
output=${wd}/all_samples_calls_contig_${contig}_filtered_with_Monomorphic.vcf

bcftools filter -i 'MIN(DP)>100 && MAX(DP)<1500 && QUAL>30' ${input} -Ou|bcftools filter -e 'F_MISSING>0.1' -Ou|bcftools filter -e 'TYPE=="indel"' -Ou|bcftools view -m1 -M2 -Ov|cat|perl /home/pkalhori/bin/vcf2minmq.pl 20|bcftools convert -Ov -o $output &
