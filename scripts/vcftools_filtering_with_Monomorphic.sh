...
output=${wd}/all_samples_reseq_chromosome_${contig}_filtered_3_with_Monomorphic
...
vcftools --gzvcf $input --remove-indels --max-missing $MISS --minQ $QUAL --min-meanDP $MIN_DEPTH --max-meanDP $MAX_DEPTH --minDP $MIN_DEPTH --maxDP $MAX_DEPTH --min-alleles 1 --max-alleles 2 --recode --out $output &
done
