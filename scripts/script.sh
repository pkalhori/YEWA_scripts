...
QUAL=30
MIN_DEPTH=5
MIN_DEPTH_INVARIANT=4
MAX_DEPTH=20

##Invariant Sites
vcftools --gzvcf $input --remove-indels --max-missing $MISS --minQ $QUAL --min-meanDP $MIN_DEPTH_INVARIANT --max-meanDP $MAX_DEPTH --minDP $MIN_DEPTH_INVARIANT --maxDP $MAX_DEPTH --max-alleles 2 --max-maf 0  --recode --stdout | bgzip -c >  ${output_Monomorphic} &

##SNPs (biallelic only)
vcftools --gzvcf $input --remove-indels --max-missing $MISS --minQ $QUAL --min-meanDP $MIN_DEPTH --max-meanDP $MAX_DEPTH --minDP $MIN_DEPTH --maf 0.05 --maxDP $MAX_DEPTH --min-alleles 2 --max-alleles 2 --hwe 0.001 --recode --stdout | bgzip -c >  ${output_SNP} &
