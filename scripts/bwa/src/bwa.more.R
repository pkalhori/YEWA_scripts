...
# run by typing:
# Rscript --verbose --vanilla bwa.more.R
# ################################################################################
...
# ################################################################################
# step one: get names and full paths of clean read files
# ################################################################################

# get a list of directories with cleaned data:
d1 <-  list.files("/home3/jdumbacher/pyrocephalus/clean/")
...
f1 <- list.files(paste0("/home3/jdumbacher/pyrocephalus/clean/",d3), full.names=TRUE) 
...
o3 <- gsub("/clean/", "/aligned/", d3, fixed=TRUE)
o3

# create a command for bwa-mem for paired end read alignments:
cmd <- paste0("/home/mvandam/bwa-mem2-2.1_x64-linux/bwa-mem2.avx2 mem -t 32 -o ",o3,"_pe_out.sam ",idxbase," ",  
...
cmd2 <- paste0("/home/mvandam/bwa-mem2-2.1_x64-linux/bwa-mem2.avx2 mem -t 32 -o ",
              o3,"_se_out.sam ",idxbase," ",f3,"_se.fq.gz")
...
for(i in 1:45) {
  listf[i] <- list(list.files(paste0("/home3/jdumbacher/pyrocephalus/clean/",d3[i]), full.names=FALSE))
  }
...
cmd <- paste0("cat ",o3,"_pe_out.sam ",o3,"_se_out.sam > ",o3,"_combined.sam")

lapply(cmd,system)

cmd2 <-  paste0("samtools view -h -b -S ",o3,"_combined.sam > ", o3,"_combined.bam")
